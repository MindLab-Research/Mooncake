...
TEST(DurableProviderRpcTest, DiscoveryKeepsBothOwnersAndUnmountRemovesOnlyOne) {
    ...
    const OffloadTaskItem task{
        .tenant_id = "default", .key = "regional-key", .size = 1024};
    ...
    ASSERT_TRUE(master.NotifyOffloadSuccess(second, {task}, {b}));
    auto both =
        master.GetReplicaListForAdmin("regional-key", TenantId::Default());
    ASSERT_TRUE(both);
    ...
    EXPECT_EQ(remaining->replicas[0].get_local_disk_descriptor().client_id,
              second);
    EXPECT_EQ(
        remaining->replicas[0].get_local_disk_descriptor().transport_endpoint,
